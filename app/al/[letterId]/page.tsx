...
export default function PublicLetterPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const found = letters.find((l: any) => l.id === params.letterId);
  ...
  if (notFound) return (
    <main style={{ minHeight: "100vh", background: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 900, color: "#1a1a2e", marginBottom: 8 }}>Letter not found</div>
  ...
  if (!letter) return <main style={{ minHeight: "100vh", background: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ color: "#D4881A" }}>Loading...</div></main>;
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f0f0f0", padding: "40px 20px" }}>
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        {expired && (
          <div style={{ background: "#FEF2F2", border: "1px solid #FCA5A5", borderRadius: 10, padding: "12px 16px", marginBottom: 16, textAlign: "center", fontSize: 13, color: "#DC2626", fontWeight: 600 }}>
            ⚠️ This pre-approval has expired. Contact your loan officer for an updated letter.
          </div>
        )}
        <div id="letter-doc" style={{ background: "white", borderRadius: 12, padding: "48px 56px", color: "#1a1a2e", fontFamily: "Georgia, serif", boxShadow: "0 2px 20px rgba(0,0,0,0.1)" }}>
          <div style={{ borderBottom: "3px solid #D4881A", paddingBottom: 20, marginBottom: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <div style={{ fontSize: 26, fontWeight: 900, color: "#D4881A", fontFamily: "'Playfair Display',serif", marginBottom: 4 }}>Pre-Approval Letter</div>
                <div style={{ fontSize: 13, color: "#666" }}>Conditional Mortgage Pre-Approval</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 12, color: "#666" }}>{fmt(letter.createdAt)}</div>
                <div style={{ fontSize: 12, color: "#888", marginTop: 3 }}>Ref: {letter.id.toUpperCase()}</div>
              </div>
            </div>
          </div>
          <div style={{ marginBottom: 28, lineHeight: 1.8 }}>
            ...
            <p style={{ fontSize: 14, marginBottom: 16, color: "#333" }}>
              This letter confirms that <strong>{letter.borrowerName}{letter.coborName ? ` and ${letter.coborName}` : ""}</strong> {letter.coborName ? "have" : "has"} been pre-approved for a <strong>{letter.loanType}</strong> mortgage loan, subject to conditions herein.
            </p>
            <div style={{ background: "#FFF8EE", border: "2px solid #F5A623", borderRadius: 12, padding: "20px 24px", margin: "24px 0" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {[["Maximum Purchase Price", `$${parseInt(letter.maxPrice).toLocaleString()}`], ["Loan Program", letter.loanType], ["Property Address", letter.propertyAddress || "To Be Determined"], ["Approval Expires", fmt(letter.expiresDate)]].map(([l, v]) => (
                  <div key={l}>
          ...
          <div style={{ borderTop: "1px solid #ddd", paddingTop: 24, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            <div>
              ...
              <div style={{ fontSize: 16, fontWeight: 700 }}>{lo?.name || "Your Loan Officer"}</div>
              {lo?.nmls_number && <div style={{ fontSize: 13, color: "#666" }}>NMLS #{lo.nmls_number}</div>}
              {lo?.email && <div style={{ fontSize: 13, color: "#D4881A" }}>{lo.email}</div>}
            </div>
            {letter.agentName && (
              <div>
                ...
                <div style={{ fontSize: 16, fontWeight: 700 }}>{letter.agentName}</div>
                {letter.agentBrokerage && <div style={{ fontSize: 13, color: "#666" }}>{letter.agentBrokerage}</div>}
              </div>
            )}
          </div>
        </div>
        ...
        <style>{`@media print{button{display:none!important}body{background:white!important}#letter-doc{box-shadow:none!important}}`}</style>
      </div>
    </main>
  );
}
